...
<div class="max-w-2xl mx-auto py-6 sm:px-6 lg:px-8">
    ...
    <form action="{{ route('kelas.update', $kelas->id) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="kode_kelas" id="kode_kelas" value="{{ old('kode_kelas', $kelas->kode_kelas) }}" required class="w-full border px-3 py-2 rounded" />
        </div>

        <div>
            <label for="nama_kelas" class="block font-medium text-gray-700">Nama Kelas</label>
            <input type="text" name="nama_kelas" id="nama_kelas" value="{{ old('nama_kelas', $kelas->nama_kelas) }}" required class="w-full border px-3 py-2 rounded" />
        </div>
